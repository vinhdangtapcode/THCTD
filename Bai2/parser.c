...
void compileStatement(void) {
  ...
  case KW_REPEAT:
    compileRepeatSt();
    break;
  // Empty statement (allow statements to be empty before END, ELSE, etc.)
  case KW_END:
  case KW_ELSE:
  case KW_UNTIL:
  case SB_SEMICOLON:
...
void compileAssignSt(void) {
  ...
  // Check for multi-variable assignment: x,y,z := exp1,exp2,exp3
  ...
  eat(SB_ASSIGN);
  compileExpression();
  // Parse multiple expressions for multi-variable assignment
  while (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    compileExpression();
  }
  assert("Assignment parsed ....");
}
